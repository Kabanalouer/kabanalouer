...
const PILLS_5 = ["Tout", "1+", "2+", "3+", "4+", "5+"];
const PILLS_4 = ["Tout", "1+", "2+", "3+", "4+"];
...
function PillGroup({ options, value, onChange }: {
  ...
  return (
    <div className="flex gap-2 flex-wrap">
      {options.map((opt) => {
        const active = opt === "Tout" ? !value : opt === `${value}+`;
        return (
          <button
            key={opt}
            onClick={() => onChange(opt === "Tout" ? "" : opt.replace("+", ""))}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
              ...
              {/* Chambres et lits */}
              <div>
                <h3 className="text-sm font-bold text-gray-900 mb-4">Chambres et lits</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-gray-500 mb-2.5">Nombre de chambres</p>
                    <PillGroup options={PILLS_5} value={minBedrooms} onChange={setMinBedrooms} />
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-2.5">Nombre de lits</p>
                    <PillGroup options={PILLS_5} value={minBeds} onChange={setMinBeds} />
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-2.5">Nombre de salles de bain</p>
                    <PillGroup options={PILLS_4} value={minBathrooms} onChange={setMinBathrooms} />
                  </div>
                </div>
              </div>
