        ...
        <div className="flex items-center gap-5">
          <div className="relative w-20 h-20 shrink-0">
            {avatarUrl ? (
              ...
              // Sans photo : tout le cercle ouvre la sélection de fichier, et la
              // pastille appareil photo rend l'action évidente.
              <button
                type="button"
                onClick={() => avatarInputRef.current?.click()}
                disabled={avatarUploading}
                aria-label={t("addPhoto")}
                className="group w-20 h-20 rounded-full bg-charcoal-100 hover:bg-charcoal-200 transition-colors flex items-center justify-center"
              >
                <span className="text-charcoal-600 font-bold text-2xl">{initial}</span>
                <span className="absolute -bottom-0.5 -right-0.5 w-8 h-8 rounded-full bg-white border border-[#dddddd] shadow-sm flex items-center justify-center group-hover:border-charcoal-300 transition-colors">
                  <svg className="w-4 h-4 text-charcoal-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75} aria-hidden="true">
