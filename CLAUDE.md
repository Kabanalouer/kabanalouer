...
- **Langue Checkout + reçus** (ajouté le 2026-07-07) : `app/api/stripe/checkout/route.ts` passe `locale` (page de paiement) et `preferred_locales` sur le Customer (reçus/factures Stripe), les deux basés sur `preferred_language` — ce sont deux réglages Stripe distincts, `locale` n'affecte pas la langue des reçus. ⚠️ `preferred_locales` n'est fixé qu'à la **création** du Customer Stripe : les customers créés avant ce correctif gardent leur langue de reçu par défaut (browser au moment du paiement). Décision volontaire de ne pas les mettre à jour rétroactivement (`stripe.customers.update()`) — à ce stade, ce ne sont que des comptes de test.
- **Rappels de renouvellement** (ajouté le 2026-07-07) : cron Vercel `/api/cron/subscription-reminders` (`vercel.json`, `0 13 * * *` — 9h Toronto en été, 8h en hiver, pas d'ajustement DST automatique côté Vercel Cron). Logique différente selon le mode de renouvellement :
  - **Offre de lancement** (`is_free_launch: true`, doit renouveler activement) : séquence de 3 rappels — 30j/10j/3j avant `expires_at` — textes et bouton orientés "Renouveler mon annonce" (`lib/emails/subscriptionReminder.ts`, `sendSubscriptionReminderEmail()`)
  - **Abonnement payant** (renouvellement automatique Stripe) : un seul email informatif à 30 jours (`sendAutoRenewalReminderEmail()`), qui précise qu'aucune action n'est requise et pointe vers le portail Stripe (gérer le paiement / annuler)
  - **Anti-doublon** : 4 colonnes sur `subscriptions` (`reminder_30d_sent`, `reminder_10d_sent`, `reminder_3d_sent`, `reminder_auto_renewal_sent`) + `reminder_cycle_expires_at`, qui mémorise à quel `expires_at` ces booléens correspondent. Le cron réinitialise seul le cycle dès que `expires_at` change (renouvellement détecté) — aucune logique de reset séparée dans le webhook Stripe ni dans `activate-free`
  - **Bouton manquant corrigé** : `/dashboard/subscription` n'offrait auparavant aucune action pour les comptes offre de lancement actifs (juste un encadré informatif) — ajout d'un bouton "Passer à l'abonnement payant" pour que le lien générique des emails fonctionne dans tous les cas
- **Paiement échoué / `past_due`** (ajouté le 2026-07-07) : `customer.subscription.updated` écrasait auparavant `past_due` en `inactive` silencieusement (impossible de distinguer "jamais abonné" de "paiement en échec", et `/dashboard/subscription` montrait à tort le bouton "S'abonner" au lieu du portail). Corrigé :
