...
- **`listings.custom_slug`** (text nullable, index UNIQUE partiel) : lien personnalisé optionnel choisi par le proprio, **un seul lien partagé entre les versions FR et EN** d'une même fiche (jamais deux liens distincts par langue). Modifiable dans le dashboard, section "Infos générales" (`components/dashboard/CustomSlugField.tsx`) — aperçu temps réel de l'URL finale, validation minuscules/chiffres/tirets seulement (max 60 caractères, jamais purement numérique pour éviter toute ambiguïté avec un listing_number), vérification d'unicité au moment de sauvegarder via `POST /api/listings/[id]/custom-slug`.
...
- **11 endroits corrigés** pour construire leurs liens avec `custom_slug ?? listing_number` au lieu de `slug_fr`/`slug_en` : `components/ListingCard.tsx`, `components/chalets/ChaletsMap.tsx`, `components/dashboard/ListingsClient.tsx`, `app/chalets/[...segments]/_components/RegionLanding.tsx` (JSON-LD `ItemList`), `app/chalets/ville/[slug]/page.tsx`, `app/page.tsx`, `app/favoris/page.tsx`, `app/dashboard/page.tsx`, `app/api/listings/geo/route.ts`, `app/api/admin/listings/[id]/publish/route.ts` (courriel de bienvenue), `app/sitemap.ts`.
...
| `add-listing-number-custom-slug.sql` | Ajoute `listings.listing_number`/`custom_slug`/`previous_custom_slug` + 2 index UNIQUE partiels, et assigne `listing_number = 48347` à la fiche déjà publiée (776cbb0b-f45f-4b0b-bea9-ebaf0ced7a72) — voir section 9, "URL de fiche chalet" | À exécuter par Simon dans Supabase SQL Editor |
