...
| `slugs-migration.sql` | Ajoute `listings.slug_fr`/`slug_en` (déjà utilisées en code depuis longtemps) **et** un index UNIQUE partiel sur chacune (`WHERE ... IS NOT NULL`, donc compatible avec les lignes existantes sans slug) — jamais exécuté, confirmé par le schéma live (2026-09-17) qui n'a aucune contrainte unique sur ces colonnes. Renforce le filet de sécurité applicatif de `ensureListingSlugs()` (voir section 9, URLs hiérarchiques). Sans danger sur les données existantes. | À exécuter par Simon dans Supabase SQL Editor |
